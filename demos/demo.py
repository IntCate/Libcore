...
# 演示输出：日志横切面与总线 ledger 都走标准 logging，这里配置到控制台
logging.basicConfig(level=logging.INFO, format="%(message)s")

from libcore.kernel.bus import Dispatch, Notice, CapabilityResult, Scope, EventBus, Aspect
from libcore.kernel.agent import ReasonProvider, Action, AgentLoop
from libcore.plugins.loader import CapabilityLoader, AspectLoader


# ---- 横切面：经 plugins/aspects（文件插件 + config/aspects.yaml 列表序）统一装配 ----
# ---- 占位能力（未来从旧 tool_kernel / services 平移） ----

_LIBROOT = Path(__file__).parent.parent / "libcore"
...
async def main() -> None:
    ...
    for r in trace.records:
        print(f"     {r.get('output_summary', r.get('event', '?'))}  {r.get('input_summary', '')}")
    ...
    print("   运行期被实际调度的 target:", [r.get("output_summary", r.get("event", "?")) for r in trace2.records])
    ...
    CapabilityLoader.write_config(hot_cfg, {"plugins": [
        ...
        {"name": "knowledge", "enabled": True, "description": "知识库检索"},
    ]})
    ...
    async def human_edits_config():
        ...
        CapabilityLoader.write_config(hot_cfg, {"plugins": [
            {"name": "pdf_tool", "enabled": True, "description": "解析 PDF"},
            {"name": "knowledge", "enabled": False},    # 改为关闭
        ]})
        await asyncio.sleep(0.14)             # 再打开一次，改为开启 + 新描述
        CapabilityLoader.write_config(hot_cfg, {"plugins": [
            {"name": "pdf_tool", "enabled": True, "description": "解析 PDF"},
            {"name": "knowledge", "enabled": True, "description": "知识库检索(新)"},
        ]})
    ...
    print("\n=== 8) 生命周期层记账：总线 ledger（运行不漏） ===")
    # 注入自定义 ledger 收集总线生命周期事件；横切面异常护栏也在其中
    events = []
    lb = EventBus(ledger=events.append)
    lb.on("tool.pdf", tool_pdf_parse)
    lb.off("tool.pdf")
    print("   总线生命周期:", [e["kind"] for e in events])

    class BoomAspect(Aspect):
        async def before(self, signal):
            raise RuntimeError("护栏故意爆炸")

    try:
        lb.add_aspect(BoomAspect())
        lb.on("tool.pdf", tool_pdf_parse)
        await lb.dispatch(Dispatch(target="tool.pdf", op="parse", payload={}))
    except RuntimeError:
        pass
    kinds = [e["kind"] for e in events]
    print("   含横切面异常护栏:", "aspect.error" in kinds)
    print("   记账事件:", kinds)
