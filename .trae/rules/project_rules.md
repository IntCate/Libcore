...
| **能力插件**   | skill/tools/mcp/context/prompt/memory/knowledge/api/ui/im/cli/model                                                                    | 渐进式披露（manifest 只暴露真实入口，按需取详情）；find/read/run 三态；`_safe_resolve` 防路径穿越                                                                                     |
...
```bash
python -m pytest tests -q        # 全量自动化测试（当前基线 111 passed）
python demos/demo.py             # 内核闭环 demo
python demos/demo_engine.py      # skill 门面闭环 demo
python demos/demo_channel_loop.py # 统一通道闭环 demo
```
...
> 当前基线：`python -m pytest tests -q` → **111 passed**（原 37 + 新增 74）。
...
3. 跑一次基线测试（`python -m pytest tests -q`），记录当前状态；
4. 对照问题清单，确认当前待修复项；
