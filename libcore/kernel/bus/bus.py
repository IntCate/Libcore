...
_logger = logging.getLogger("libcore")


def _default_ledger(event: dict) -> None:
    """总线生命周期记账的默认后端：写标准 logging（DEBUG 级，默认安静）。

    只"记一笔"，不参与投递、不决策。可注入自定义回调（文件 / JSON / DB）
    替换输出端，内核无需改动。要看生命周期事件，把 ``libcore`` logger
    调到 DEBUG，或注入自定义 ledger 收集。
    """
    _logger.debug("ledger %s", event)
...
class EventBus:
    ...
    def __init__(self, ledger: Callable[[dict], None] | None = None) -> None:
        ...
        # 生命周期记账钩子：bus.on/off、横切面挂载、异常护栏等"非信号"事件，
        # 由总线顺手记一笔（默认写 logging，可注入替换）。保证系统运行不漏。
    ...
    def add_aspect(self, aspect: Aspect) -> None:
        ...
        self._ledger({"kind": "bus.aspect_added", "aspect": type(aspect).__name__})
    ...
    def remove_aspect(self, aspect: Aspect) -> None:
        ...
        try:
            ...
            return
        self._ledger({"kind": "bus.aspect_removed", "aspect": type(aspect).__name__})
    ...
    async def _run_aspects(self, phase: str, signal: Signal, result: Any = None) -> Any:
        ...
        for aspect in chain:
            ...
            try:
                if phase == "before":
                    step_r = await step(signal)
                else:
                    await step(signal, result)
                    step_r = None
            except Exception as e:  # noqa: BLE001 - 横切面故障必须被记账，系统运行不漏
                self._ledger({
                    "kind": "aspect.error",
